...
export class AdoptionService {
  ...
  getAdoptionPost(): Observable<AdoptionsDetail[]> {
    return this.firestore.collection<AdoptionsDetail>(`adoptionPost`, ref => ref.orderBy('createdAt', 'desc')).valueChanges();
  }
  ...
  ownerAdoptionDetail(adoptionId: string, userId: string): Observable<AdoptionsDetail> {
    return this.firestore.collection('adoption').doc(userId).collection('adoptionDetail' , ref => ref.orderBy('createdAt', 'desc')).doc<AdoptionsDetail>(adoptionId).valueChanges();
  }
  ...
  getPendingList(userId: string): Observable<AdoptionsRequest[]> {
    return this.firestore.collection('adoption').doc(userId).collection<AdoptionsRequest>('adoptionRequest' , ref => ref.orderBy('createdAt', 'desc')).valueChanges();
  }
  ...
  getApplicationList(userId: string): Observable<AdoptionsRequest[]> {
    return this.firestore.collection('adoption').doc(userId).collection<AdoptionsRequest>('adoptionApplication', ref => ref.orderBy('createdAt', 'desc')).valueChanges();
  }
